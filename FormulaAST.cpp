...
namespace ASTImpl {
	...
	// бит устанавливается, когда требуются круглые скобки
	enum PrecedenceRule {
		PR_NONE = 0b00,                // никогда не нуждался
		PR_LEFT = 0b01,                // необходимо для оставленного ребенка
		PR_RIGHT = 0b10,               // необходимо для правильного ребенка
		PR_BOTH = PR_LEFT | PR_RIGHT,  // необходимо для обоих детей
	};

	// PRECEDENCE_RULES[родительский][дочерний] определяет, нужны ли круглые скобки
	// должен быть вставлен между родителем и дочерним элементом определенных прецедентов;
	// для некоторых узлов правила отличаются для левых и правых дочерних элементов:
	// (X c Y) p Z  vs  X p (Y c Z)
	//
	// Интересными случаями являются те, когда удаление скобок привело бы к изменению AST.
